...
var _blgr = _interopRequireDefault(require("blgr"));

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

const NetAddress = _fcoin.net.NetAddress,
      packets = _fcoin.net.packets;
...
class Peer {
  ...
  async connect() {
    let log = new _blgr.default({
      level: 'spam'
    }); // Open up the logger

    await log.open(); // Create the Fcoin Peer
    ...
    let address = NetAddress.fromHostname(this.settings.ip); // Fire off the initial connection attempt

    this.peer.connect(address); // When we recieve packets from peers, process them using the onPacket function

    this.peer.parser.on('packet', this.onPacket.bind(this)); // Handle/log errors
    ...
    if (this.connected) {
      try {
        ...
        for (let hex of hexArray) {
          ...
          let mytx = new _fcoin.TX().fromRaw(Buffer.from(hex, 'hex')); // Store the hash of the transaction in our txMap

          this.txMap[mytx.hash('hex')] = hex; // Add it to our array to announce
  ...
  handleGetData(getDataPacket) {
    ...
    for (let item of getDataPacket.items) {
      ...
      let regTX = item.type === _fcoin.InvItem.types.TX;
      let segTX = item.type === _fcoin.InvItem.types.WITNESS_TX; // If we are a regular tx, or a segwit tx, then continue
      ...
      if (regTX || segTX) {
        ...
        let mytx = new _fcoin.TX().fromRaw(Buffer.from(this.txMap[item.hash.toString('hex')], 'hex')); // Include the transaction in a TXPacket to be sent to the Peer

        let txPacket = new packets.TXPacket(mytx, segTX); // Send out the TXPacket to the Peer
