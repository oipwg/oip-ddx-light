...
const MIN_UTXO_AMOUNT = 1; // Prevent chaining over ancestor limit

const MAX_MEMPOOL_ANCESTORS = 1225;
const MAX_MEMPOOL_ANCESTOR_SIZE = 1.50 * ONE_MB; // Timer lengths used to track and fix the Ancestor chain
...
class RPCWallet {
  constructor(options) {
    ...
    this.lastTXTime = Date.now(); // Initialize our initial peer array
  ...
  async checkAllUnconfirmed() {
    ...
    while (this.unconfirmedTxids.length !== 0 && !foundUnconfirmed) {
      let txidToCheck = this.unconfirmedTxids[0]; // Check to see if the utxo is still in the mempool and if it has ancestors

      let getMempoolEntry = await this.rpcRequest('getmempoolentry', [txidToCheck]); // Check if we have an error and handle it
      ...
      if (getMempoolEntry.error && getMempoolEntry.error !== null) {
        ...
        if (getMempoolEntry.error.message === 'Transaction not in mempool' || getMempoolEntry.error.message === 'Transaction not in mempool.') {
          ...
          let checkConfirmations = await this.rpcRequest('getrawtransaction', [txidToCheck, true]); // Ignore if there was an error, but set the number of confirmations if available

          let confirmations;

          if (checkConfirmations.result && checkConfirmations.result.confirmations) {
            confirmations = checkConfirmations.result.confirmations;
  ...
  async updateAncestorStatus() {
    console.log('[RPC Wallet] Updating Ancestor Status...'); // Check all transactions still inside of our unconfirmed array

    await this.checkAllUnconfirmed(); // Using the unconfirmed array, sum up the current Ancestor total
  ...
  async addAncestor(hex) {
    ...
    this.currentAncestorSize += hex.length; // Every 100 update our count

    if (this.currentAncestorCount % 100 === 0) {
  ...
  async checkAncestorCount(forceUpdateAncestor) {
    ...
    while (this.currentAncestorCount >= MAX_MEMPOOL_ANCESTORS || this.currentAncestorSize >= MAX_MEMPOOL_ANCESTOR_SIZE || forceFirstLoopUpdate) {
      this.checkingAncestorCount = true; // Only force an update for the first loop, then disable to prevent looping forever
    ...
    this.checkingAncestorCount = false; // Count the number of confirmed
  ...
  async rebroadcastTransactions() {
    ...
    this.rebroadcasting = true; // Announce that we are starting

    console.log(`[RPC Wallet] Announcing ${this.unconfirmedTransactions.length} transactions to Peers...`); // Destroy any peers we have currently connected to to get a "clean" rebroadcast

    await this.destroyPeers(); // Connect to Peers to use for the rebroadcast
  ...
  async sendTX(inputs, outputs, floData) {
    ...
    this.unconfirmedTxids.push(broadcastTX.result); // Add the tx we just sent to the Ancestor count

    await this.addAncestor(signedTXHex); // Set the new tx to be used as the next output.

    this.previousTXOutput = {
      txid: broadcastTX.result,
      amount: outputs[this.publicAddress],
      vout: 0 // Mark a timestamp for the last time we have sent a transaction,
      // this is to allow us to check if we should be running `checkAncestorCount`
      // inside of the `onConfirmationInterval` check

    };
    this.lastTXTime = Date.now(); // Return the TXID of the transaction
  ...
  onConfirmation(_ref, options) {
    ...
    this.onConfirmationSubscriptions[record.getTXID()] = {
      txids,
      record,
      options // If we do not already have a loop going to make sure confirmations get fired off, create one
  ...
  async checkForConfirmations() {
    ...
    for (let subscription in this.onConfirmationSubscriptions) {
      let _this$onConfirmationS = this.onConfirmationSubscriptions[subscription],
  ...
  async waitForConfirmations() {
    ...
    while (subCount > 0) {
      ...
      await new Promise((resolve, reject) => {
        setTimeout(resolve, 10 * ONE_SECOND);
      });
      await this.checkAncestorCount(true);
      await this.checkForConfirmations();
