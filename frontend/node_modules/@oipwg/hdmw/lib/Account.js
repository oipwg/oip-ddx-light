...
var _bip32Utils = _interopRequireDefault(require("@oipwg/bip32-utils"));
...
class Account {
    ...
    if (serializedData) {
      ...
      if (serializedData.addresses) {
        ...
        for (const address of serializedData.addresses) {
          rehydratedAddresses.push(new _Address.default(address.wif, this.coin, address));
        }
  ...
  getAddress(chainNumber, addressNumber) {
    ...
    const tmpHydratedAddr = new _Address.default(addr, this.coin, false); // Attempt to match to address that we already have
  ...
  getNextChainAddress(chainNumber) {
    return new _Address.default(this.account.getChain(chainNumber || 0).next(), this.coin, false);
  }
  /**
  ...
  sendPayment(options) {
    return new Promise((resolve, reject) => {
      ...
      const processPayment = () => {
        const sendFrom = [];
        const allAddresses = this.getAddresses(); // Check if we define what address we wish to send from
        ...
        const txb = new _TransactionBuilder.default(this.coin, newOpts);
  ...
  async DiscoverChain(chainNumber, gapLimit) {
    ...
    try {
      discovered = await (0, _util.discovery)(chain, gapLimit, this.ChainPromise, chainNumber, this.coin);
    } catch (e) {
  ...
  async ChainPromise(addresses, coin) {
    ...
    for (const addr of addresses) {
      const address = new _Address.default(addr, coin, false);
      const addressUpdatePromise = address.updateState(); // This will only be called for any rejections AFTER the first one,
    ...
    for (const address of promiseResponses) {
      results[address.getPublicAddress()] = address.getTotalReceived() > 0; // Store all addresses
