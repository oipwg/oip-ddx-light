...
var _bitcoinjsMessage = _interopRequireDefault(require("@oipwg/bitcoinjs-message"));
...
class Address {
  ...
  getPublicAddress() {
    ...
    if (this.fromBIP32 && this.address) {
      ...
      if (!publicKey && this.address.getPublicKeyBuffer) {
        publicKey = this.address.getPublicKeyBuffer();
      }
    }

    return this.fromBIP32 ? (0, _util.toBase58)(publicKey, this.coin.network.pubKeyHash) : this.pubAddress;
  }
  /**
  ...
  signMessage(message) {
    ...
    try {
      signatureBuffer = _bitcoinjsMessage.default.sign(message, privateKeyBuffer, compressed, messagePrefix);
    } catch (e) {
  ...
  verifySignature(message, signature) {
    ...
    try {
      valid = _bitcoinjsMessage.default.verify(message, this.getPublicAddress(), signature, this.coin.network.messagePrefix);
    } catch (e) {
