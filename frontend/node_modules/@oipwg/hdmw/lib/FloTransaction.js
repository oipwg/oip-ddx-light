...
class FloTransaction {
  ...
  static fromBuffer(buffer, _NO_STRICT) {
    ...
    if (marker === FloTransaction.ADVANCED_TRANSACTION_MARKER && flag === FloTransaction.ADVANCED_TRANSACTION_FLAG) {
      hasWitnesses = true;
    } else {
  ...
  isCoinbase() {
    return this.ins.length === 1 && FloTransaction.isCoinbaseHash(this.ins[0].hash);
  }
  ...
  byteLength(_ALLOW_WITNESS = true) {
    ...
    return (hasWitnesses ? 10 : 8) + (this.version >= 2 ? varSliceSize(this.floData) : 0) + varuint.encodingLength(this.ins.length) + varuint.encodingLength(this.outs.length) + this.ins.reduce((sum, input) => {
  ...
  hashForWitnessV0(inIndex, prevOutScript, value, hashType) {
    ...
    if (!(hashType & FloTransaction.SIGHASH_ANYONECANPAY) && (hashType & 0x1f) !== FloTransaction.SIGHASH_SINGLE && (hashType & 0x1f) !== FloTransaction.SIGHASH_NONE) {
    ...
    if ((hashType & 0x1f) !== FloTransaction.SIGHASH_SINGLE && (hashType & 0x1f) !== FloTransaction.SIGHASH_NONE) {
    ...
    } else if ((hashType & 0x1f) === FloTransaction.SIGHASH_SINGLE && inIndex < this.outs.length) {
  ...
  __toBuffer(buffer, initialOffset, _ALLOW_WITNESS = false) {
    ...
    const bufferWriter = new bufferutils.BufferWriter(buffer, initialOffset || 0);
...
class FloPsbt {
  ...
  signAllInputsHD(hdKeyPair, sighashTypes = [transaction.Transaction.SIGHASH_ALL]) {
  ...
  signAllInputsHDAsync(hdKeyPair, sighashTypes = [transaction.Transaction.SIGHASH_ALL]) {
  ...
  signInputHD(inputIndex, hdKeyPair, sighashTypes = [transaction.Transaction.SIGHASH_ALL]) {
    if (!hdKeyPair || !hdKeyPair.publicKey || !hdKeyPair.fingerprint) {
      throw new Error('Need HDSigner to sign input');
    }
  ...
  signInputHDAsync(inputIndex, hdKeyPair, sighashTypes = [transaction.Transaction.SIGHASH_ALL]) {
    return new Promise((resolve, reject) => {
      if (!hdKeyPair || !hdKeyPair.publicKey || !hdKeyPair.fingerprint) {
        return reject(new Error('Need HDSigner to sign input'));
      }
  ...
  signAllInputs(keyPair, sighashTypes = [transaction.Transaction.SIGHASH_ALL]) {
  ...
  signAllInputsAsync(keyPair, sighashTypes = [transaction.Transaction.SIGHASH_ALL]) {
    return new Promise((resolve, reject) => {
      if (!keyPair || !keyPair.publicKey) {
  ...
  signInput(inputIndex, keyPair, sighashTypes = [transaction.Transaction.SIGHASH_ALL]) {
    if (!keyPair || !keyPair.publicKey) {
      throw new Error('Need Signer to sign input');
    }
  ...
  signInputAsync(inputIndex, keyPair, sighashTypes = [transaction.Transaction.SIGHASH_ALL]) {
    return new Promise((resolve, reject) => {
      if (!keyPair || !keyPair.publicKey) {
        return reject(new Error('Need Signer to sign input'));
      }
...
class FloPsbtTransaction {
  ...
  addInput(input) {
    ...
    const hash = typeof input.hash === 'string' ? bufferutils.reverseBuffer(Buffer.from(input.hash, 'hex')) : input.hash;
...
function hasSigs(neededSigs, partialSig, pubkeys) {
  ...
  if (pubkeys) {
    sigs = pubkeys.map(pkey => {
      const pubkey = ecpair.fromPublicKey(pkey, {
        compressed: true
      }).publicKey;
...
function getHashForSig(inputIndex, input, cache, sighashTypes) {
  ...
  const sighashType = input.sighashType || FloTransaction.SIGHASH_ALL;
...
function sighashTypeToString(sighashType) {
  let text = sighashType & FloTransaction.SIGHASH_ANYONECANPAY ? 'SIGHASH_ANYONECANPAY | ' : '';
