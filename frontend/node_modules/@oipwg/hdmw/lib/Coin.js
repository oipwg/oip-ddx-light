...
class Coin {
  ...
  constructor(node, coin, options) {
    ...
    let bip44Num = this.coin.network.slip44; // Check if we need to convert the hexa to the index
    ...
    if (serializedData) {
      if (serializedData.accounts) {
        for (const accountNumber in serializedData.accounts) {
          ...
          this.accounts[accountNumber] = new _Account.default(accountMaster, this.coin, {
            discover: false,
  ...
  async getBalance(options) {
    ...
    const accountsToSearch = []; // Check if we are an array (ex. [0,1,2]) or just a number (ex. 1)
  ...
  sendPayment(options) {
    return new Promise((resolve, reject) => {
      ...
      const processPayment = () => {
        let sendFrom = [];
        let allAddresses = []; // Add all Addresses from selected accounts to array
  ...
  getAccount(accountNumber) {
    ...
    if (typeof accountNumber === 'string' && !isNaN(parseInt(accountNumber))) {
      num = parseInt(accountNumber);
    }
  ...
  addAccount(accountNumber, discover) {
    ...
    this.accounts[num] = new _Account.default(accountMaster, this.coin, {
