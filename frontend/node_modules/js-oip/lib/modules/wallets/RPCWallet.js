...
class RPCWallet {
  constructor(options) {
    ...
    this.previousTXOutput = undefined; // Initialize our initial peer array
  ...
  async rpcRequest(method, parameters) {
    ...
    try {
      ...
      rpcRequest = await this.rpc.post("http://".concat(this.options.rpc.host, ":").concat(rpcPort, "/"), {
        'jsonrpc': '2.0',
        'id': 1,
        'method': method,
        'params': parameters
  ...
  async updateAncestorStatus() {
    ...
    let utxos = await this.getUTXOs(); // Grab the most recent txid

    let mostRecentUTXO = utxos[0];
    let mostRecentTXID = mostRecentUTXO.txid; // Check to see if the utxo is still in the mempool and if it has ancestors

    let getMempoolEntry = await this.rpcRequest('getmempoolentry', [mostRecentTXID]); // Check if we have an error and handle it
    ...
    if (getMempoolEntry.error && getMempoolEntry.error !== null) {
      ...
      if (getMempoolEntry.error.message === 'Transaction not in mempool' || getMempoolEntry.error.message === 'Transaction not in mempool.') {
        ...
        let checkConfirmations = await this.rpcRequest('getrawtransaction', [mostRecentTXID, true]); // Ignore if there was an error, but set the number of confirmations if available
    ...
    if (getMempoolEntry.result) {
      let txMempoolStatus = getMempoolEntry.result; // Store the current ancestor count & size
  ...
  addAncestor(hex) {
    ...
    if (this.currentAncestorCount % 50 === 0) {
      console.log("[RPC Wallet] Updated Ancestor Count: ".concat(this.currentAncestorCount, " - Updated Ancestor Size: ").concat((this.currentAncestorSize / ONE_MB).toFixed(2), "MB"));
    }
  ...
  async checkAncestorCount() {
    ...
    let startAncestorSize = this.currentAncestorSize; // Variables to track loop state
    ...
    while (this.currentAncestorCount >= MAX_MEMPOOL_ANCESTORS || this.currentAncestorSize >= MAX_MEMPOOL_ANCESTOR_SIZE) {
      ...
      let hasMaxAncestors = this.currentAncestorCount >= MAX_MEMPOOL_ANCESTORS || this.currentAncestorSize >= MAX_MEMPOOL_ANCESTOR_SIZE; // Only log ancestor count if it is the first loop, and we still have too many ancestors

      if (hasMaxAncestors && firstLoop) {
        console.log("[RPC Wallet] Maximum Unconfirmed Transaction count reached, pausing sending of transactions until some of the current transactions get confirmed | Ancestor Count: ".concat(this.currentAncestorCount, " - Ancestor Size: ").concat((this.currentAncestorSize / ONE_MB).toFixed(2), "MB"));
      ...
      if (hasMaxAncestors && Date.now() - REPAIR_ANCESTORS_AFTER > reachedAncestorLimitTimestamp) {
        ...
        console.log("[RPC Wallet] [WARNING] Unconfirmed Transaction count has not decreased in ".concat(REPAIR_ANCESTORS_AFTER / ONE_MINUTE, " minutes, rebroadcasting transactions in an attempt to repair the utxo chain!")); // Attempt the actual repair
    ...
    let numberConfirmed = startAncestorCount - this.currentAncestorCount; // Remove the transactions that just got confirmed
    ...
    if (startAncestorCount >= MAX_MEMPOOL_ANCESTORS || startAncestorSize >= MAX_MEMPOOL_ANCESTOR_SIZE) {
      console.log("[RPC Wallet] ".concat(numberConfirmed, " Transactions Confirmed! (").concat(((startAncestorSize - this.currentAncestorSize) / ONE_MB).toFixed(2), " MB)")); // If there are less confirmed than REPAIR_MIN_TX, then rebroadcast the transactions
    ...
    if (hadMaxAncestors) {
      console.log("[RPC Wallet] Unconfirmed count has decreased, resuming sending transactions! | Ancestor Count: ".concat(this.currentAncestorCount, " - Ancestor Size: ").concat((this.currentAncestorSize / ONE_MB).toFixed(2), "MB"));
  ...
  async rebroadcastTransactions() {
    ...
    console.log("[RPC Wallet] Announcing ".concat(this.unconfirmedTransactions.length, " transactions to Peers...")); // Connect to Peers to use for the rebroadcast
    ...
    for (let txHex of this.unconfirmedTransactions) {
      ...
      if (numAnnounced % 50 === 0) {
        console.log("[RPC Wallet] Announced ".concat(numAnnounced, "/").concat(this.unconfirmedTransactions.length, " transactions so far..."));
      }
    }

    console.log("[RPC Wallet] Announced ".concat(numAnnounced, " transactions!")); // Wait for REBROADCAST_LENGTH in order to give some time for transactions to be requested, and sent out.
  ...
  async initialize() {
    ...
    console.log("[RPC Wallet] Loading ".concat(this.currentAncestorCount, " transactions into local mempool, please wait... (this may take a little while)"));
    /* Update the tx chain */
    // First, we need to get a list of unconfirmed transactions, to do this, we need the most recent utxo.

    let utxos = await this.getUTXOs(); // Then, while we have ancestors, lookup the transaction hex, and add it to the start of the unconfirmed transaction chain
    ...
    while (nextTXID) {
      ...
      this.unconfirmedTransactions.unshift(txHex.result); // Then lookup to see if it is in the mempool
      ...
      if (this.unconfirmedTransactions.length % 50 === 0) {
        console.log("[RPC Wallet] Loaded ".concat(this.unconfirmedTransactions.length, "/").concat(this.currentAncestorCount, " transactions into local mempool so far..."));
      }
    }

    console.log("[RPC Wallet] Loaded ".concat(this.unconfirmedTransactions.length, " transactions into local mempool!")); // Return true, signifying that the initialization was successful
  ...
  async connectToPeers() {
    ...
    console.log("[RPC Wallet] Connecting to ".concat(getPeerInfo.result.length, " Peers...")); // For each of those peers, open up a connection
    ...
    for (let peerInfo of getPeerInfo.result) {
      ...
      console.log("peer host ".concat(peerHost));
      let peer = new _Peer.default({
        ip: peerHost,
    ...
    console.log("[RPC Wallet] Connected to ".concat(connectedPeers, " Peers!"));
  }
  /**
  ...
  async signMessage(message) {
    ...
    let myECPair = _bitcoinjsLib.ECPair.fromWIF(this.wif, this.coin.network);

    let privateKeyBuffer = myECPair.privateKey; // Check if we are a compress privKey

    let compressed = myECPair.compressed || true; // Create the actual signature
    ...
    let signature = signatureBuffer.toString('base64'); // Return the base64 signature
  ...
  async getUTXOs() {
    ...
    let utxoRes = await this.rpcRequest('listunspent', [MIN_CONFIRMATIONS, MAX_CONFIRMATIONS, [this.publicAddress]]); // Throw if there was an error
    ...
    let utxos = utxoRes.result; // Filter out transactions that don't meet our minimum UTXO value
  ...
  async sendDataToChain(floData) {
    ...
    let input = utxos[0]; // Calculate the minimum Transaction fee for our transaction by counting the size of the inputs, outputs, and floData
    ...
    let txid = await this.sendTX([input], output, floData); // Returns the TXID of the transaction if sent successfully
  ...
  createAndSignTransaction(input, outputs, floData) {
    ...
    let txb = new _FLOTransactionBuilder.default(this.coin.network); // Add our single input
    ...
    try {
      ...
      throw new Error("Unable to build Transaction Hex!: ".concat(err));
  ...
  async sendTX(inputs, outputs, floData) {
    ...
    if (typeof floData !== 'string') {
      throw new Error("Data must be of type string. Got: ".concat(typeof floData));
    }

    if (floData.length > _FLOTransaction.FLODATA_MAX_LEN) {
      throw new Error("Error: 'floData' length exceeds ".concat(_FLOTransaction.FLODATA_MAX_LEN, " characters. Please send a smaller data package."));
    } // Make sure that we don't have too many ancestors. If we do, then waits for some transactions to be confirmed.


    await this.checkAncestorCount(); // Create and sign the transaction hex

    let signedTXHex = this.createAndSignTransaction(inputs[0], outputs, floData); // Broadcast the transaction hex we created to the network

    let broadcastTX = await this.rpcRequest('sendrawtransaction', [signedTXHex]); // Check if there was an error broadcasting the transaction
    ...
    this.unconfirmedTransactions.push(signedTXHex); // Set the new tx to be used as the next output.
