...
var ie = _interopRequireWildcard(require("@oipwg/insight-explorer"));
...
class ExplorerWallet {
  ...
  constructor(options) {
    ...
    if (options.explorerUrl) {
      network.explorer = new ie.Insight(options.explorerUrl);
    }
  ...
  async sendDataToChain(data) {
    ...
    try {
      ...
      throw new Error("Error building TX Hex: ".concat(err));
    }
    ...
    try {
      ...
      throw new Error("Error broadcasting TX Hex: ".concat(err));
  ...
  async buildTXHex() {
    ...
    try {
      ...
      throw new Error("Failed to build inputs and outputs: ".concat(err));
    }
    ...
    let {
      inputs,
      outputs
    } = selected; // inputs and outputs will be undefined if no solution was found

    if (!inputs || !outputs) {
      throw new Error('No Inputs or Outputs selected! Fail!');
    }

    let txb = new _FLOTransactionBuilder.default(this.network);
    ...
    outputs.forEach(output => {
      if (!output.address) {
        throw new Error("Missing output address: ".concat(outputs));
      }
    ...
    for (let i in inputs) {
      if (this.p2pkh !== inputs[i].address) throw new Error("Invalid inputs. Addresses don't match: ".concat(inputs, " & ").concat(this.p2pkh));
  ...
  async buildInputsAndOutputs() {
    ...
    try {
      ...
      throw new Error("Failed to get utxo: ".concat(err));
    ...
    if (utxo.length === 0) {
      ...
      while (utxo.length === 0 && finish < 6000) {
        ...
        try {
          ...
          throw new Error("Failed to get utxo from insight explorer: ".concat(err));
        }
    ...
    if (utxo.length === 0) {
      ...
      if (formattedUtxos.length === 0) {
        throw new Error("P2PKH: ".concat(this.p2pkh, " has no unspent transaction outputs"));
      }
    } else {
    ...
    let targets = outputs;
    let extraBytes = this.coin.getExtraBytes({
      floData
    });
  ...
  async getUTXO() {
    ...
    try {
      ...
      throw new Error("Error fetching UTXOs: ".concat(err));
  ...
  createManualUtxos() {
    ...
    for (let f of floTxs) {
      ...
      for (let i = 0; i < outputs.length; i++) {
        ...
        let {
          hash
        } = _bitcoinjsLib.address.fromBase58Check(addr);

        let testnetAddr = _bitcoinjsLib.address.toBase58Check(hash, 115);
  ...
  async broadcastRawHex(hex) {
    ...
    try {
      ...
      throw new Error("Failed to broadcast TX Hex: ".concat(err));
    }
