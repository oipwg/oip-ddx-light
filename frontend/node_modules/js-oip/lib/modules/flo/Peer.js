...
class Peer {
  ...
  async connect() {
    ...
    let address = _fcoin.netaddress.fromHostname(this.settings.ip); // Fire off the initial connection attempt
  ...
  announceTX(hex) {
    ...
    if (this.connected) {
      try {
        ...
        let mytx = (0, _fcoin.tx)().fromRaw(Buffer.from(hex, 'hex')); // Store the hash of the transaction in our txMap
  ...
  handleGetData(getDataPacket) {
    ...
    if (getDataPacket.items.length > 1 || this.singleLog < 5) {
      console.log("[RPC Wallet] Peer ".concat(this.settings.ip, " requested ").concat(getDataPacket.items.length, " items..."));
    ...
    for (let item of getDataPacket.items) {
      ...
      if (regTX || segTX) {
        ...
        let mytx = (0, _fcoin.tx)().fromRaw(Buffer.from(this.txMap[item.hash], 'hex')); // Include the transaction in a TXPacket to be sent to the Peer

        let txPacket = _fcoin.packets.TXPacket(mytx, segTX); // Send out the TXPacket to the Peer
    ...
    if (txsRelayed > 0) {
      ...
      console.log("[RPC Wallet] Relayed ".concat(txsRelayed, " requested transactions to ").concat(this.settings.ip, " (last txid ").concat(lastHash, ")"));
    }
  }
